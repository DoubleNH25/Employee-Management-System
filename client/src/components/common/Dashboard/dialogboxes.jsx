...
export const AddEmployeesDialogBox = () => {
    ...
    return (
        <div className="AddEmployees-content">
            <Dialog>
                <DialogTrigger className="bg-purple-600 border-2 border-purple-600 md:px-4 md:py-2 md:text-lg min-[250px]:px-2 min-[250px]:py-1 min-[250px]:text-sm text-white font-bold rounded-lg hover:bg-white hover:text-purple-600">Add Employees</DialogTrigger>
                <DialogContent className="max-w-[315px] sm:max-w-[50vw] 2xl:max-w-[45vw]">
                    <DialogHeader>
                        <DialogTitle>Add Employee Info</DialogTitle>
...
export const EmployeeDetailsDialogBox = ({ EmployeeID }) => {
    ...
    const FetchEmployeeData = (EmID) => {
        const employee = HREmployeesState.data.find((item) => item.id === EmID)
        return employee
    }
    ...
    return (
        <div className="Employees-Details-container">
            <Dialog>
                <div>
                    <DialogTrigger className="btn-sm btn-blue-700 text-md border-2 border-purple-600 min-[250px]:px-2 min-[250px]:py-1 sm:px-1 sm:py-0.5 xl:px-2 xl:py-1 rounded-md hover:bg-purple-600 hover:text-white">View</DialogTrigger>
                </div>
                <DialogContent className="max-w-[315px] lg:max-w-[55vw] 2xl:max-w-[45vw]">
                    ...
                    <div className="employee-data-container flex flex-col gap-4">
                        <div className="employee-profile-logo flex items-center gap-3">
                            <div className="logo border-2 border-purple-600 rounded-[50%] flex justify-center items-center">
                                <p className="font-bold text-2xl text-purple-600 p-2">{`${employeeData.firstname.slice(0, 1).toUpperCase()} ${employeeData.lastname.slice(0, 1).toUpperCase()}`}</p>
                            </div>
                        ...
                        <div className="employees-all-details grid lg:grid-cols-2 min-[250px]:gap-2 lg:gap-10">
                            ...
                            <div className="details-group-1 flex flex-col gap-3">
                                <div className="label-value-pair flex items-center gap-2">
                                    <label className="font-bold md:text-sm xl:text-lg">Tasks Assigned :</label>
                                    <p className="md:text-sm xl:text-lg">{employeeData.taskIds ? employeeData.taskIds.length : 0}</p>
                                </div>
                                <div className="label-value-pair flex items-center gap-2">
                                    <label className="font-bold md:text-sm xl:text-lg">Work Schedule :</label>
                                    <p className="md:text-sm xl:text-lg">{employeeData.schedule ? `${employeeData.schedule.workHours?.start || 'N/A'} - ${employeeData.schedule.workHours?.end || 'N/A'}` : 'Not Set'}</p>
                                </div>
                                <div className="label-value-pair flex items-center gap-2">
...
export const DeleteEmployeeDialogBox = ({ EmployeeID }) => {
    ...
    return (
        <div className="delete-employee-dialog-container">
            <Dialog>
                <DialogTrigger className="btn-sm btn-blue-700 text-md border-2 border-purple-600 min-[250px]:px-2 min-[250px]:py-1 sm:px-1 sm:py-0.5 xl:px-2 xl:py-1 rounded-md hover:bg-purple-600 hover:text-white">Delete</DialogTrigger>
...
export const CreateDepartmentDialogBox = () => {
    ...
    const CreateDepartment = async () => {
        const result = await dispatch(HandlePostHRDepartments({ apiroute: "CREATE", data: formdata }))
      
        if (result.type === 'HandlePostHRDepartments/fulfilled') {
            setformdata({
                name: "",
                description: ""
            })
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger className="min-[250px]:text-sm sm:text-lg min-[250px]:px-2 min-[250px]:py-1 sm:px-4 sm:py-2 bg-purple-600 font-bold text-white rounded-lg border-2 border-purple-600 hover:bg-white hover:text-purple-600">Create Department</DialogTrigger>
            <DialogContent className="max-w-[315px] lg:max-w-[35vw] 2xl:max-w-[30vw]">
                <DialogHeader>
                        ...
                        <div className="create-department-button flex justify-center items-center">
                            {
                                (formdata.name.trim().length === 0 || formdata.description.trim().length === 0) ? 
                                    <Button className="btn-sm btn-blue-700 text-md border-2 bg-purple-600 border-purple-600 px-2 py-1 rounded-md hover:bg-white hover:text-purple-600" onClick={() => ShowToast()}>Create</Button> :
                                    <Button className="btn-sm btn-blue-700 text-md border-2 bg-purple-600 border-purple-600 px-2 py-1 rounded-md hover:bg-white hover:text-purple-600" onClick={() => CreateDepartment()}>Create</Button>
                            }
                        </div>
                    </div>
                </div>
...
export const EmployeesIDSDialogBox = ({ DepartmentID }) => {
    ...
    const SetEmployees = () => {
        dispatch(HandlePatchHRDepartments({ apiroute: "ADD_EMPLOYEES", data: SelectedEmployeesData }))
    ...
    return (
        <div className="employeeIDs-box-container">
            <Dialog>
                <DialogTrigger className="px-4 py-2 font-bold m-2 bg-purple-600 text-white border-2 border-purple-600 rounded-lg hover:bg-white hover:text-purple-600 min-[250px]:text-xs md:text-sm lg:text-lg" onClick={() => dispatch(fetchEmployeesIDs({ apiroute: "GETALL" }))}>Add Employees</DialogTrigger>
                <DialogContent className="max-w-[315px] lg:max-w-[35vw] 2xl:max-w-[30vw]">
                    <DialogHeader>
                    ...
                    {EmployeesIDState.isLoading ? <Loading height={"h-auto"} /> : <div className="employeeID-checkbox-container flex flex-col gap-4">
                                        ...
                                        {EmployeesIDState.data ? EmployeesIDState.data.map((item, index) => <CommandItem key={index}>
                                            <div className="employeeID-checkbox flex justify-center items-center gap-2">
                                                <input 
                                                    ...
                                                    onChange={() => SelectEmployees(item.id)} 
                                                    checked={SelectedEmployeesData.employeeIDArray.includes(item.id)} 
                        ...
                        <div className="employeeID-checkbox-button-group flex justify-center items-center gap-2">
                            <Button className="btn-sm btn-blue-700 text-md border-2 bg-purple-600 border-purple-600 px-2 py-1 rounded-lg hover:bg-white hover:text-purple-600" onClick={() => SetEmployees()}>Add</Button>
                            <DialogClose asChild>
                                <Button className="btn-sm btn-blue-700 text-md border-2 bg-purple-600 border-purple-600 px-2 py-1 rounded-lg hover:bg-white hover:text-purple-600" onClick={() => ClearSelectedEmployeesData()}>Cancel</Button>
                            </DialogClose>
                        </div>
                    </div>}
...
export const RemoveEmployeeFromDepartmentDialogBox = ({ DepartmentName, DepartmentID, EmployeeID }) => {
    ...
    const RemoveEmployee = (EMID) => {
        dispatch(HandlePatchHRDepartments({ apiroute: "REMOVE_EMPLOYEES", data: { departmentID: DepartmentID, employeeIDArray: [EMID] } }))
    }

    return (
        <div className="remove-employee">
            <Dialog>
                <DialogTrigger className="btn-sm btn-blue-700 text-md border-2 border-purple-600 min-[250px]:px-2 min-[250px]:py-1 sm:px-1 sm:py-0.5 xl:px-2 xl:py-1 rounded-md hover:bg-purple-600 hover:text-white">Remove</DialogTrigger>
                <DialogContent className="max-w-[315px] lg:max-w-[35vw] 2xl:max-w-[30vw]">
                    <DialogHeader>
                        <DialogTitle>Remove Employee</DialogTitle>
                        <DialogDescription>Remove employee from department</DialogDescription>
                    </DialogHeader>
                    <div className="flex flex-col justify-center items-center gap-4">
...
export const UpdateDepartmentDialogBox = ({ departmentId, currentName, currentDescription }) => {
    const { toast } = useToast()
    const dispatch = useDispatch()
    const [open, setOpen] = useState(false)
    const [formdata, setformdata] = useState({
    ...
    const UpdateDepartment = async () => {
        const result = await dispatch(HandlePatchHRDepartments({ 
            apiroute: `UPDATE.${departmentId}`, 
            data: formdata 
        }))        
        if (result.type === 'HandlePatchHRDepartments/fulfilled') {
            setOpen(false)
            setTimeout(() => {
                window.location.reload();
            }, 500);
        } else {
            console.log('Department update failed or pending');
        }
    }
    ...
    const ShowToast = () => {
        toast({
            variant: "destructive",
            title: "Uh oh! Something went wrong.",
            description: `All Fields are required to update a department`,
        })
    }
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="bg-purple-600 text-white font-bold text-sm hover:bg-purple-700 w-full">
            ...
            <DialogContent className="max-w-[315px] lg:max-w-[35vw] 2xl:max-w-[30vw]">
                ...
                <div className="update-department-container flex flex-col gap-4">
                    <div className="update-department-form flex flex-col gap-4">
                        <div className="form-group flex flex-col gap-3">
                            <div className="label-input-field flex flex-col gap-1">
                                <label htmlFor="update-departmentname" className="md:text-md lg:text-lg font-bold">Department Name</label>
                                <input type="text"
                            ...
                            <div className="label-input-field flex flex-col gap-1">
                                <label htmlFor="update-departmentdescription" className="md:text-md lg:text-lg font-bold">Department Description</label>
                                <textarea
                                    id="update-departmentdescription"
                                    name="description"
                                    value={formdata.description}
                                    onChange={handleformchange}
                                    className="border-2 border-gray-700 rounded px-2 py-1 h-[100px]"
                        ...
                        <div className="update-department-button flex justify-center items-center">
                            {
                                (formdata.name.trim().length === 0 || formdata.description.trim().length === 0) ? 
                                    <Button className="btn-sm btn-blue-700 text-md border-2 bg-purple-600 border-purple-600 px-2 py-1 rounded-md hover:bg-white hover:text-purple-600" onClick={() => ShowToast()}>Update</Button> :
                                    <Button className="btn-sm btn-blue-700 text-md border-2 bg-purple-600 border-purple-600 px-2 py-1 rounded-md hover:bg-white hover:text-purple-600" onClick={() => UpdateDepartment()}>Update</Button>
                            }
                        </div>
                    </div>
                </div>
...
export const DeleteDepartmentDialogBox = ({ departmentId, departmentName, employeeCount }) => {
    const dispatch = useDispatch()
    const [open, setOpen] = useState(false)

    const DeleteDepartment = async () => {
        const result = await dispatch(HandleDeleteHRDepartments({ 
            apiroute: `DELETE.${departmentId}` 
        }))        
        if (result.type === 'HandleDeleteHRDepartments/fulfilled') {
            setOpen(false)
            setTimeout(() => {
                window.location.reload();
            }, 500);
        } else {
            console.log('Department deletion failed or pending');
        }
    }
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="bg-red-700 text-white font-bold text-sm hover:bg-red-900 w-full">
            ...
            <DialogContent className="max-w-[315px] lg:max-w-[35vw] 2xl:max-w-[30vw]">
                ...
                <div className="flex flex-col justify-center items-center gap-4">
                    <div className="text-center">
                        <p className="text-lg font-bold mb-2">Are you sure you want to delete "{departmentName}"?</p>
                        {employeeCount > 0 && (
                            <p className="text-sm text-orange-600 font-semibold">
                                ⚠️ This department has {employeeCount} employee(s). They will be removed from this department.
                            </p>
                        )}
                    </div>
